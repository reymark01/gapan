...
<?php
		...
		while($row = $results->fetch()) {
			echo '<div class="col-sm-3"><div class="card"><div class="card-header card-container" style="background-color:white;"><a href="event/'.$row['id'].'">
					<img class="thumbnail" src="/events_thumbnail/'.$row['events_thumbnail'].'">';
			echo '<h3 class="card-title">'.$row['events_title'].'</h3>
				<small class="text-muted">'.Validate::formatDate($row['events_postdate']).'</small></a></div></div></div>';
		}
		echo '</div>
			</div>';
?>
