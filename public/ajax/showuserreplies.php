	...
	if (!empty(Input::get('commentid'))) {
		if (!empty(Input::get('first'))) {
			if (Session::exist('u_sess_id') || Session::exist('b_sess_id')) {
				echo '<div class="container">
						<form class="form-group ureply-form">
						<textarea class="form-control ureplyarea" name="reply"></textarea>
			...
			$sql = "SELECT user_reply.id, user_reply.u_reply, user_reply.u_replydate, stores.b_name, stores.b_profile, stores.b_username, users.fname, users.lname, users.username, users.profile FROM ((user_reply LEFT JOIN stores ON user_reply.store_id = stores.id) LEFT JOIN users ON user_reply.user_id = users.id) WHERE user_reply.user_comment_id = :commentid ORDER BY user_reply.id DESC LIMIT 0, 2";
			$results = DB::query($sql, [], true, ['commentid' => Input::get('commentid')]);
			echo '<div class="uappendreply">';
		...
		while($row = $results->fetch()) {
			...
			if (!empty($row['b_username'])) {
			 ...
			 echo '<br><small class="text-muted">'.Validate::formatDate($row['u_replydate']).'</small>
			 <br><br><div class="posttext">'.str_replace('  ', ' &nbsp;', nl2br($row['u_reply'])).'<hr></div>
			 </div>';
		}
		if (!empty(Input::get('first'))) {
			echo '</div>
				<a href="#" class="uviewmorereplies">view more replies</a>
				<a href="#" class="uviewallreplies">view all replies</a>';
		}
	}
}