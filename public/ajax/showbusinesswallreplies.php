	...
	if (!empty(Input::get('commentid'))) {
		if (!empty(Input::get('first'))) {
			if (Session::exist('u_sess_id') || Session::exist('b_sess_id')) {
				echo '<div class="container">
						<form class="form-group wreply-form">
						<textarea class="form-control wreplyarea" name="wreply"></textarea>
			...
			$sql = "SELECT store_wall_reply.id, store_wall_reply.bw_reply, store_wall_reply.bw_replydate, stores.b_name, stores.b_profile, stores.b_username, users.fname, users.lname, users.username, users.profile FROM ((store_wall_reply LEFT JOIN stores ON store_wall_reply.store_id = stores.id) LEFT JOIN users ON store_wall_reply.user_id = users.id) WHERE store_wall_reply.store_wall_comment_id = :commentid ORDER BY store_wall_reply.id DESC LIMIT 0, 2";
			$results = DB::query($sql, [], true, ['commentid' => Input::get('commentid')]);
			echo '<div class="wappendreply">';
		...
		while($row = $results->fetch()) {
			...
			if (!empty($row['b_username'])) {
			 ...
			 echo '<br><small class="text-muted">'.Validate::formatDate($row['bw_replydate']).'</small>
			 <br><br><div class="posttext">'.str_replace('  ', ' &nbsp;', nl2br($row['bw_reply'])).'<hr></div>
			 </div>';
		}
		if (!empty(Input::get('first'))) {
			echo '</div>
				<a href="#" class="wviewmorereplies">view more replies</a>
				<a href="#" class="wviewallreplies">view all replies</a>';
		}
	}
}