...
if (Input::exist()) {
	if (!empty(Input::get('pushreply'))) {
		...
		if (Session::exist('u_sess_id')) {
			...
			if (DB::query($sql2, ['b_notif_from' => 'user', 'b_notif_type' => 'comment', 'b_link' => 'post', 'b_linkto' => 'wallpost'], true, ['store_id' => $storeid['id'], 'b_from_id' => Session::get('u_sess_id'), 'b_link_id' => $row['pid']])) {
			 	$pusher->trigger('bAddNotifyChannel', 'bAddNotifyEvent', ['b_id' => $storeid['id'], 'count' => 1]);
			}
			$sql3 = "SELECT user_id, store_id FROM store_wall_post_comments WHERE id = :id";
			$result = DB::query($sql3, [], true, ['id' => $row['cid']])->fetch();
			$sql5 = "SELECT b_username FROM stores, store_wall_post WHERE stores.id = store_wall_post.store_id AND store_wall_post.id = :postid";
			...
			if (!empty($result['user_id'])) {
				if ($result['user_id'] != Session::get('u_sess_id')) {
					...
					DB::query($sql4, ['notif_from' => 'user', 'notif_type' => 'commentreply', 'link' => $username['b_username'], 'usertype' => 'business', 'linkto' => 'wallpost'], true, ['userid' => $result['user_id'], 'from_id' => Session::get('u_sess_id'), 'link_id' => $row['pid']]);
				}
				...
				DB::query($sql4, ['b_notif_from' => 'user', 'b_notif_type' => 'commentreply', 'b_link' => $username['b_username'], 'b_usertype' => 'business', 'b_linkto' => 'wallpost'], true, ['store_id' => $result['store_id'], 'b_from_id' => Session::get('u_sess_id'), 'b_link_id' => $row['pid']]);
			}
			...
			$out .= '<br><small>'.Validate::formatDate($row['bw_replydate']).'</small>
			 <br><br><div class="posttext">'.str_replace('  ', ' &nbsp;', nl2br($row['bw_reply'])).'</div></div>';
			...
			if (Session::get('b_sess_id') != $storeid['id']) {
				$sql2 = "INSERT INTO store_notification (store_id, b_notif_from, b_from_id, b_notif_type, b_link, b_linkto, b_link_id) VALUES (:store_id, :b_notif_from, :b_from_id, :b_notif_type, :b_link, :b_linkto, :b_link_id)";
				if (DB::query($sql2, ['b_notif_from' => 'store', 'b_notif_type' => 'reply', 'b_link' => 'post', 'b_linkto' => 'wallpost'], true, ['store_id' => $storeid['id'], 'b_from_id' => Session::get('b_sess_id'), 'b_link_id' => $row['pid']])) {
					$pusher->trigger('bAddNotifyChannel', 'bAddNotifyEvent', ['b_id' => $storeid['id'], 'count' => 1]);
				}
			}
			$sql3 = "SELECT user_id, store_id FROM store_wall_post_comments WHERE id = :id";
			...
			if (!empty($result['user_id'])) {
				$sql4 = "INSERT INTO user_notification (user_id, notif_from, from_id, notif_type, linkusertype, link, linkto, link_id) VALUES (:userid, :notif_from, :from_id, :notif_type, :usertype, :link, :linkto, :link_id)";
				DB::query($sql4, ['notif_from' => 'store', 'notif_type' => 'commentreply', 'link' => $username['b_username'], 'usertype' => 'business', 'linkto' => 'wallpost'], true, ['userid' => $result['user_id'], 'from_id' => Session::get('b_sess_id'), 'link_id' => $row['pid']]);
			} elseif (!empty($result['store_id'])) {
				if ($result['store_id'] != Session::get('b_sess_id')) {
					$sql4 = "INSERT INTO store_notification (store_id, b_notif_from, b_from_id, b_notif_type, b_linkusertype, b_link, b_linkto, b_link_id) VALUES (:store_id, :b_notif_from, :b_from_id, :b_notif_type, :b_usertype, :b_link, :b_linkto, :b_link_id)";
					DB::query($sql4, ['b_notif_from' => 'store', 'b_notif_type' => 'commentreply', 'b_link' => $username['b_username'], 'b_usertype' => 'business', 'b_linkto' => 'wallpost'], true, ['store_id' => $result['store_id'], 'b_from_id' => Session::get('b_sess_id'), 'b_link_id' => $row['pid']]);
				}
			}
		}
	}
