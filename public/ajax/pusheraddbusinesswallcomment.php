	...
	if (!empty(Input::get('pushcomment'))) {
		...
		if (Session::exist('u_sess_id')) {
			...
			$out .= '<br><small class="text-muted">'.Validate::formatDate($row['bw_commentdate']).'</small>
			 <br><br><div class="posttext">'.str_replace('  ', ' &nbsp;', nl2br($row['bw_comment'])).'</div></div>';
			...
			$out .= '<br><small>'.Validate::formatDate($row['bw_commentdate']).'</small>
			 <br><br><div class="posttext">'.str_replace('  ', ' &nbsp;', nl2br($row['bw_comment'])).'</div></div>';
			...
			$sql2 = "INSERT INTO store_notification (store_id, b_notif_from, b_from_id, b_notif_type, b_link, b_linkto, b_link_id) VALUES (:store_id, :b_notif_from, :b_from_id, :b_notif_type, :b_link, :b_linkto, :b_link_id)";
			if (DB::query($sql2, ['b_notif_from' => 'store', 'b_notif_type' => 'comment', 'b_link' => 'post', 'b_linkto' => 'wallpost'], true, ['store_id' => $storeid['id'], 'b_from_id' => Session::get('b_sess_id'), 'b_link_id' => $row['pid']])) {
					$pusher->trigger('bAddNotifyChannel', 'bAddNotifyEvent', ['b_id' => $storeid['id'], 'count' => 1]);
				}
			}
		}
