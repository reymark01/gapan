...
$result = DB::query($sql, [], true, ['id' => Input::get('nid')])->fetch();
if (empty($result)) {
	Redirect::to('/admin/news-post');
}
if (Input::exist()) {
  if (Token::check('newsPostEditToken', Input::get('token'))) {
    ...
    if ($validation->passed()) {
      ...
      $sql = "UPDATE news SET news_title = :title, news_post = :post, news_thumbnail = :postphoto WHERE id = :id";
      if (DB::query($sql, ['title' => Input::get('title'), 'post' => Input::get('body'), 'postphoto' => $key], true, ['id' => $result['id']])) {
        if (!empty($_FILES['file']['name'])) {
          if (Input::get('thumbnail') != 'default') {
          	$path = '../news_thumbnail/'.$result['news_thumbnail'];
          	unlink($path);
          }
          move_uploaded_file($tmp_name, $newsphotos);
        }
        Session::flash('newsPostEditSuc', 'Success');
      }
      Redirect::to('/admin/news-post');
    }
  }
...
<div class="row">
  ...
  <div class="col-sm-10 border border-dark shadow p-3 mb-5 bg-white rounded" style="margin-top: 55px;">
    <form class="form-group" action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" class="form-control" name="title" value="<?=$result['news_title']?>">
      <label><b>Current Thumbnail:</b></label>
<?php
		if ($result['news_thumbnail'] != 'default') {
			echo '<img src="/news_thumbnail/'.$result['news_thumbnail'].'" style="padding:10px; width: 200px; height: 200px;">';
		} else {
			echo 'no thumbnail';
		}
?>
	  <input type="hidden" name="thumbnail" value="<?=$result['news_thumbnail']?>">
	  <br>
      <label><b>Change Post Thumbnail</b></label>
      <input type="file" class="form-control" name="file">
      <br>
      <label><b>Post Body</b></label>
      <textarea class="form-control" name="body"><?=$result['news_post']?></textarea>
      <br>
      <input type="hidden" name="token" value="<?php echo Token::generate('newsPostEditToken') ?>">
