...
$results =  DB::query("SELECT * FROM user_post WHERE u_postverified = 0");
  while ($row = $results->fetch()) {
	  ...
	  $result = DB::query($sql, [], true, ['id' => $row['user_id']])->fetch();
    $sql2 = "SELECT * FROM user_post_photos WHERE user_post_id = :postid";
    ...
    <form action="" method="post">
      ...
      echo 'No Image<br>';
        ...
        foreach($photos as $photo) {
          echo '<img class="img-thumbnail" src="/user_photos/'.$photo['u_postphoto'].'" style="width: 160px; height: 160px;">';
        }
        echo '</div>';
      }
      ?>
